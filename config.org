...
#+end_src
** python-shell
The python shell is a comint inferior shell implemented in built-in =python-mode=. At this time, it
only seems to work will with python version 3.10. There needs to be a shell command on the system
PATH named =python3.10=. Also add a hook so that all python shells start with eglot to ensure code
completion, etc.
#+begin_src emacs-lisp

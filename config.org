...
=clojure-mode=,  =elisp-mode=, and =cider-repl-mode= major modes only.
#+BEGIN_SRC emacs-lisp
  (use-package paredit
    :ensure t
    :pin melpa-stable
...
*** Diminish minor modes from the modeline
    ...
    that here by using the =:diminish= keyword in =use-package= declarations that we want to suppress.
#+BEGIN_SRC emacs-lisp

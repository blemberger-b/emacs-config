...
The initial Emacs frame in GUI mode will open as a maximized window.
#+begin_src emacs-lisp
...
#+end_src
** Pairing Parenthesis
Enable built-in =electric-pair-mode=, which automatically creates the matching pair of parenthesis,
brackets, or braces.
#+begin_src emacs-lisp
...
Set the default column to 80 where emacs will attempt to auto-fill in those modes that support it.
#+BEGIN_SRC emacs-lisp
...
minibuffer when using =counsel-M-x= to run commands. [[https://stable.melpa.org/#/ivy-hydra][ivy-hydra]] shows useful keys/commands in the
minibuffer when invoking counsel/swiper, etc by typing =C-o=.
#+begin_src emacs-lisp
  (use-package ivy
    :diminish
    :bind (("C-s" . swiper)
...
#+end_src
* Bookmarks
** Limit bookmark scope to current project
...
#+end_src
** Keybinding to rename bookmarks
This overrides the default binding for =move-to-window-line-top-bottom= which is a navigation
function that I never use.
#+begin_src emacs-lisp
  (general-def "M-r" 'bookmark-rename)
#+end_src
* Mode Line
...
** Work Org Mode Files
One work computer, set a variable for my documents folder. 
#+begin_src emacs-lisp
  (setq one-drive-dir (getenv "ONEDRIVE_DIR"))
#+end_src
** UI Tweaks
...
whether the tags are already in the file or not. Format is a association list with the tag as a
string and a character as the value, where the character is the shortcut key.
#+begin_src emacs-lisp
  (setq org-tag-alist
        '(("story" . ?s)
          ("meeting" . ?m)
...
*** Keybinding for =org-agenda-list=
=org-agenda-list= is the daily/weekly agenda view.
#+begin_src emacs-lisp
  (general-define-key "M-`" 'org-agenda-list)
#+end_src
*** Start =org-mode= with agenda logging.
#+begin_src emacs-lisp
  (setq org-agenda-start-with-log-mode t)
#+end_src
*** Filter closed scheduled tasks out of agenda view
#+begin_src emacs-lisp
...
a setting below. ~customize-set-variable~ instead of ~setq~ prevents the value of the custom
variable being written to ~init.el~.
#+begin_src emacs-lisp
...
*** Define custom agenda views
    ...
    #+end_src
*** Refiling
Refiling provides for a way to quickly move tasks from one heading/file to another.
#+begin_src emacs-lisp
  (setq org-refile-targets
        '((nil :maxlevel . 1)
          ("work.org_archive" :maxlevel . 1)
          ("personal.org_archive" :maxlevel . 1)))
#+end_src
...
#+end_src
** Capture Settings
Capture mode is invoked in any buffer with a keybinding defined below. It is useful for making quick
lists. Keep separate capture lists for personal and work usage. Enter the specified key prefix to
choose either.
#+begin_src emacs-lisp
  (let ((work-capture-file (concat one-drive-dir "/work-notes/work.org")))
    (setq org-capture-templates
      (list (list "w" "Work Inbox" 'entry (list 'file+headline work-capture-file "Inbox")
                  "* TODO %?\n  %i")
            '("p" "Personal Inbox" entry (file+headline "~/repos/personal/gtd/personal.org" "Inbox")
              "* TODO %?\n  %i"))))
#+end_src
Bind a quick key for capture of "Work Inbox" items.
#+begin_src emacs-lisp
  (bsl/control-c-definer "I"
    (lambda () (interactive) (org-capture nil "w")))
#+end_src
Bind a quick key for capture of "Personal Inbox" items.
#+begin_src emacs-lisp
  (bsl/control-c-definer "P"
...
#+END_SRC
** Export Formats
Customize org-mode to support the =md= backend for markdown, as well as some
default ones.
#+begin_src emacs-lisp
...
#+end_src
* PDF
This mode will open PDF files in emacs and provide full search/mark/copy
functionality.
#+begin_src emacs-lisp
  (use-package pdf-tools
      ...
      :hook ((pdf-view-mode . (lambda () (auto-revert-mode -1)))
             (pdf-view-mode . (lambda () (company-mode -))))
...
in all programming language files with this setting enabled.
...
** Syntax Checking
=flymake= is an on-the-fly syntax checker built in to emacs that interfaces with an LSP
server. Here, we turn off its mode-line indicator.
#+begin_src emacs-lisp
  (require 'flymake)
  (setq flymake-mode-line-format nil)
  (bsl/control-c-definer :keymaps 'flymake-mode-map
    "F p" 'flymake-goto-prev-error
    "F n" 'flymake-goto-next-error)
#+end_src
** Eldoc Documentation Help
=eldoc= is a built-in emacs feature that shows relevant documentation for a symbol at point. Here,
we make sure that the pop-up documentation in the echo area does not exceed 1 line.
#+begin_src emacs-lisp
  (require 'eldoc)
  (setq eldoc-echo-area-use-multiline-p nil)
#+end_src
** Debugger
The built-in emacs debugger =GUD= seems to have some problems, so a better one
named [[https://github.com/realgud/realgud][realgud]] is available on MELPA. Change the default PDB debugger command to
always use Python 3.
#+begin_src emacs-lisp
  (use-package realgud
    :pin melpa-stable
...
#+end_src
This package installs interfaces to popular debuggers like =pdb=, =gdb=, and =lldb=.
** Treesitter
[[https://www.masteringemacs.org/article/how-to-get-started-tree-sitter][tree-sitter]] is a built-in package in Emacs 29 that, when activated, creates a
syntax tree for a source code buffer.

*Prerequisites:* 
 - Emacs 29 or higher must be installed. You can check if tree-sitter is
   supported in your version of emacs by evaluating the *non-interactive* elisp function
   ~treesit-available-p~. 
 - You /must/ have an /ABI compatible/ version of the [[https://github.com/tree-sitter/tree-sitter-python/tree/v0.23.6][tree-sitter python grammar
   library]] installed. To see the ABI version of the treesitter support in your
   version of emacs, evaluating the non-interactive elisp function ~treesit-library-abi-version~.
 - Find a compatible version of [[https://github.com/tree-sitter/tree-sitter-python/tree/v0.23.6][tree-sitter-python]] from the documenation [[https://tree-sitter.github.io/tree-sitter/using-parsers/7-abi-versions.html#abi-versions][here]].
 - From the [[https://github.com/tree-sitter/tree-sitter-python/tree/v0.23.6][tree-sitter-python]] gitlab repo, pick a tag corresponding to the desired
   version number, and note the *tag* value (i.e. copy to clipboard).
 - In emacs, run the interactive command
   ~treesit-install-language-grammar~. Enter =python= for the language, and use
   the defaults for all prompts except for the tag, where you should specify the
   value from above.
 - Restart Emacs
 - To confirm that python's tree-sitter is working, evaluate the
   *non-interactive* elisp expression ~(treesit-language-available-p 'python)~

Here, we replace the default =python-mode=
with the =python-ts-mode= that has built-in support for treesitter.
#+begin_src emacs-lisp
    (setq major-mode-remap-alist
          '((python-mode . python-ts-mode)))
#+end_src
*Note:* Keybindings still need to be done for =python-ts-mode-map= rather than
 =python-mode=map= and hooks need to be made for =python-ts-mode-hook= rather
 than =python-mode-hook=.

*Note:* Someday the author may release [[https://github.com/mickeynp/combobulate][combobulate]] to MELPA, a python tree-sitter based
package, that provides many ease of use features for python development. On that
day, try installing it now that the hard part of tree-sitter support is set up.

* Language Server Protocol Servers
Use built-in emacs [[https://joaotavora.github.io/eglot/][eglot]] package for managing programming language features. 
#+begin_src emacs-lisp
  (require 'eglot)
  ;; Map super-r to eglot-rename to rename all symbols in buffer
...
#+end_src
* Python
** Consistent tab widths & indentation
Prevent =python-mode= from trying to guess the tab width when it opens a file. Instead, the above
defined =tab-width= default variable will be always used.
#+begin_src emacs-lisp
  (setq python-indent-guess-indent-offset nil) ; default is t
#+end_src
** python-shell
The python shell is a comint inferior shell implemented in built-in
=python-mode=. At this time, it only works with the python REPL that shipped
with version 3.10. There needs to be an environment variable set as such:
#+begin_src bash
  export PYTHON_BASIC_REPL=1
#+end_src
to ensure that the older REPL is used with python shell. Also add a hook so that
all python shells start with eglot to ensure code completion, etc.
#+begin_src emacs-lisp
  (setq python-shell-interpreter "python3")
  (add-hook 'inferior-python-mode-hook 'eglot-ensure)
#+end_src
** LSP Support for Python
LSP interaction is managed through =eglot=. A python LSP server must be installed in the system
PATH. A good choice to use is =jedi-language-server=. Eglot is disabled by default in all python
buffers. To enable it use this key:
#+begin_src emacs-lisp
  (bsl/control-c-definer :keymaps 'python-ts-mode-map "M-j" 'eglot)
#+end_src
To disconnect and kill the LSP server, use this key:
#+begin_src emacs-lisp
  (bsl/control-c-definer :keymaps 'python-ts-mode-map "M-q" 'eglot-shutdown)
#+end_src
** PEP 8 Reformatter
~black~ must be installed, it is available via ~pipx~ (~pipx --global install
black~).
#+begin_src emacs-lisp
...
#+end_src
** PEP 8 On-The Fly Linting
~flake8~ must be installed, it is available in Homebrew. This command is
executed and output is shown in a separate buffer when the =python-check=
command is run (bound to =C-c C-v=).
#+begin_src emacs-lisp
  (setq-default python-check-command
                "flake8 --color never --max-line-length 88")
#+end_src
** Python Debugging
=C-c D= will open a python [[https://docs.python.org/3/library/pdb.html][PDB]] debugging session
#+begin_src emacs-lisp
  (bsl/control-c-definer :keymaps 'python-ts-mode-map "D" 'realgud:pdb)
#+end_src
** Virtual Environment Handling
Eglot works to interact with the LSP server for all built-in python code, but any packages that are
installed via a virtual environment are not made available to the LSP server by default. This
package tries to do that by allowing you to run =pyvenv-activate= and select your virtual
environment directory. The activate/deactivate hooks run when a virtual env is activated/deactivated
using pyenv and is done to support the virtual environment in new =python-shell= sessions.
#+begin_src emacs-lisp
  (use-package pyvenv
    :ensure t
    :config
    ...
    (setq pyvenv-post-deactivate-hooks
          (list (lambda ()
                  (setq python-shell-interpreter "python3")))))
#+end_src
Function to activate =uv='s default virtual environment
#+begin_src emacs-lisp
  (defun bsl/activate-uv-venv ()
      (interactive)
      (let ((venv-candidate-dir (concat (projectile-project-root) ".venv")))
        (if (file-directory-p venv-candidate-dir) 
          (pyvenv-activate venv-candidate-dir)
          (message "%s not found!" venv-candidate-dir))))
#+end_src

Set some python mode specific key bindings for activating and deactivating the virtual environment.
#+begin_src emacs-lisp
  (bsl/control-c-definer :keymaps 'python-ts-mode-map "M-A" 'pyvenv-activate) ; prompt for virtual env
  (bsl/control-c-definer :keymaps 'python-ts-mode-map "M-a" 'bsl/activate-uv-venv)
  (bsl/control-c-definer :keymaps 'python-ts-mode-map "M-d" 'pyvenv-deactivate)
#+end_src
* Clojure
...
These keybindings work in both C language mode and Makefile mode. ~compile~ command in =cc-mode=
will work on both Linux and Mac OS, since on Mac, the ~gcc~ command is installed by default and is
an "alias" for LLVM's =clang=. To debug, run the appropriate debug command below for =gcc=: ~gdb~ or
for =clang=: ~lldb~.
#+begin_src emacs-lisp
...
#+end_src
** LSP Support for C/C++
LSP interaction is managed through =eglot=. A C/C++ LSP server must be installed in the system
PATH. A good choice to use is =clangd=, which is installed by default on Mac OS. Eglot is disabled
by default in all C/C++ buffers. To enable it use this key:
#+begin_src emacs-lisp
  (bsl/control-c-definer :keymaps 'c-mode-base-map "M-j" 'eglot)
#+end_src
To disconnect and kill the LSP server, use this key:
#+begin_src emacs-lisp
  (bsl/control-c-definer :keymaps 'c-mode-base-map "M-q" 'eglot-shutdown)
#+end_src
* Kotlin
I use Kotlin mostly as a DSL for Gradle.
#+begin_src emacs-lisp
...
- =treemacs-hide-gitignored-files-mode= hides i=.gitignore=d files by default. Can be turned off
  with the toggle =t i=
#+begin_src emacs-lisp

  (defun bsl/close-treemacs ()
    (interactive)
  ...
  (use-package treemacs
    :pin melpa
    :demand t
    :bind (("C-c C-`" . treemacs-select-window)

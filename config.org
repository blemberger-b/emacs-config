...
*** Diminish minor modes from the modeline
    =diminish= allows us to list the mode names that we don't want listed in the modeline. We do
...
=clojure-mode=,  =elisp-mode=, and =cider-repl-mode= major modes only. Also remap the sexp /slurp/
...
*** Cider Defaults
Suppress warning that the REPL gives when you're current buffer is not part of a leiningen project.
#+BEGIN_SRC emacs-lisp
  (setq cider-allow-jack-in-without-project t)
#+END_SRC

Suppress displaying REPL evaluation results in the clojure file's buffer, instead dispay in the
botom echo area.
#+BEGIN_SRC emacs-lisp

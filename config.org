...
Elpy works best with projects managed in a python virtual environment, so since I use *poetry* to
manage my virtualenvs, configure elpy to use the one configured by poetry.

To use poetry projects with Elpy, I need to add these PyPI packages to /every/ project in it's
poetry *dev dependencies* configuration:

#+begin_src toml
  [tool.poetry.group.elpy.dependencies]
  # These stupid dependencies need to be put in each project in order for elpy IDE in emacs to work
  # since elpy is configured to use the virtualenv provided by poetry for each project
  jedi = "*"
  black = "*"
#+end_src

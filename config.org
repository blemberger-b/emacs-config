...
#+begin_src emacs-lisp
  (use-package general
    ...
    (general-create-definer bsl/control-c-definer
      :prefix "C-c")
    (general-create-definer bsl/control-c-c-definer
      :prefix "C-c c"))
#+end_src
*** Keybindings
...
#+begin_src emacs-lisp
  ...
  (bsl/control-c-c-definer
    "e" 'config-visit    ; C-c c e
    "r" 'config-reload)  ; C-c c r
#+end_src
...
#+end_src
* Frames
The functions in this section were excerpted from the [[https://www.emacswiki.org/emacs/frame-cmds.el][frame-cmds]] Emacs Wiki library.
...
#+begin_src emacs-lisp
  ...
  ;;; Frame Resizing
  (defun bsl/enlarge-frame-vert (&optional increment frame)
    (interactive "p")
    (bsl/adjust-frame-param '+ 'height increment frame))
  (defun bsl/shrink-frame-vert (&optional increment frame)
    (interactive "p")
    (bsl/adjust-frame-param '- 'height increment frame))
  (defun bsl/enlarge-frame-horiz (&optional increment frame)
    (interactive "p")
    (bsl/adjust-frame-param '+ 'width increment frame))
  (defun bsl/shrink-frame-horiz (&optional increment frame)
    (interactive "p")
    (bsl/adjust-frame-param '- 'width increment frame))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun bsl/move-frame-up (&optional frame)
    (interactive)
    (bsl/move-frame '- 'top frame))

  (defun bsl/move-frame-down (&optional frame)
    (interactive)
    (bsl/move-frame '+ 'top frame))

  (defun bsl/move-frame-left (&optional frame)
    (interactive)
    (bsl/move-frame '- 'left frame))

  (defun bsl/move-frame-right (&optional frame)
    (interactive)
    (bsl/move-frame '+ 'left frame))
#+end_src
...
#+begin_src emacs-lisp
  (defhydra hydra-frame-resize ()
    "Frame resize"
    ("s" bsl/shrink-frame-vert "shorter")
    ("t" bsl/enlarge-frame-vert "taller")
    ("n" bsl/shrink-frame-horiz "narrower")
    ("w" bsl/enlarge-frame-horiz "wider")
...
#+begin_src emacs-lisp
  (defhydra hydra-frame-motion ()
    "Frame motion"
    ("<up>" bsl/move-frame-up "up")
    ("<down>" bsl/move-frame-down "down")
    ("<left>" bsl/move-frame-left "left")
    ("<right>" bsl/move-frame-right "right"))
#+end_src

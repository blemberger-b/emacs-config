...
The =[[https://github.com/abo-abo/hydra]hydra]]= package allows commands to be bound to short
keybindings (called "heads"). After the command is invoked using the normal keybinding, repetitive
invocations of the command can be accomplished with a single key.
#+begin_src emacs-lisp
  (require 'hydra) ;; package is brought in by ivy
...
#+begin_src emacs-lisp
  (use-package general
    ...
    :config
    (general-define-key
     :keymaps '(global org-mode-map)  ;; override keymappings from other modes
     "C-c j" nil) ;; need to undefine any bindings to use as a prefix
    (general-create-definer bsl/control-c-definer
      :prefix "C-c")
    (general-create-definer bsl/control-c-j-definer
      :prefix "C-c j"))
#+end_src
*** Keybindings
...
#+begin_src emacs-lisp
  ...
  (bsl/control-c-j-definer
    "/" 'goto-last-change
    "C-k" 'kill-all-buffers
...
#+END_SRC
Bind it to some difficult key combination to press by accident: =C-c C-j C-k=
* Windows

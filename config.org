...
#+begin_src emacs-lisp
  (use-package recentf
    ...
    (setq recentf-max-saved-items 100
          recentf-exclude (list (concat user-emacs-directory ".cache/.*")))
    (recentf-mode 1))
#+end_src
** Dashboard
...
#+begin_src emacs-lisp
  (defun bsl/kill-buffer ()
    (interactive)
    (if (xor (eq (treemacs-current-visibility) 'visible) (eq (count-windows nil) 2))
...
#+end_src
* Gnuplot
Requires =gnuplot= executable to be installed in the =PATH=.
#+begin_src emacs-lisp
  (use-package gnuplot
    :pin melpa-stable
    :mode ("\\.gp$" . gnuplot-mode)
    ;; we shouldn't need to do this but we do
    :hook (gnuplot-mode . god-local-mode))
#+end_src
* Docker

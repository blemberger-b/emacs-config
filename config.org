...
[[https://github.com/emacsorphanage/god-mode][here]]. The =ESC= key is used to toggle on/off =god-mode= in a buffer, while =i= can go out of
=god-mode= (like "insert") when in =god-mode=. Note that =god=mode= is not
enabled for all buffer types (e.g. magit, dashboard, etc.).
#+begin_src emacs-lisp
...
Binds C-<arrow-key> to move focus between windows.
...
#+begin_src emacs-lisp
  (use-package windmove
    ...
    (switch-to-buffer-obey-display-actions 1))
#+end_src
...
and /barf/ commands to keybindings that work in both GUI and TTY. Also need to unbind the
=C-<right>= and =C-<left>= bindings because they interfere with =windmove=.
#+BEGIN_SRC emacs-lisp
  (use-package paredit
    :pin melpa-stable
    :hook ((clojure-mode
    ...
    :config (unbind-key "C-<right>" paredit-mode-map)
